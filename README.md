...
The browser pane currently provides URL entry, back/forward/reload/stop, standard context menus, persistent website data, and developer tools entry points. Browser shortcuts include Cmd-L for the URL bar, Cmd-R reload, Cmd-Shift-R reload ignoring cache, Cmd-. stop loading, Cmd-[ back, Cmd-] forward, and Cmd-Option-I DevTools. By default it uses the WebKit fallback so normal app launch stays stable. The dynamically loaded CEF/Chromium backend is available behind `GSDE_ENABLE_CEF=1` / `make run-cef`; it initializes CEF, creates a native browser view, starts renderer helpers, and loads the initial page.
