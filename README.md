...
Configured terminal panes can declare a startup `command` or a named `process` from a `procfile` resolved relative to the project directory launched with `gsde`. Configured browser panes share CEF's persistent global browser profile beside the resolved config file under `.config/gsde/chromium/Default` (or `chromium/Default` next to an explicit `GSDE_CONFIG` file); GSDE creates these directories as needed. Configured `vscode` panes are isolated by pane ID with separate code-server managers, ports, and CEF profiles under `chromium/vscode-panes/<pane-id>`. By default they use native VS Code user data and extensions (`~/Library/Application Support/Code` and `~/.vscode/extensions`); set `profile = "local"` to use project-local code-server state. Use [`docs/vscode-pane-manual-verification.md`](docs/vscode-pane-manual-verification.md) as the explicit first-version manual acceptance checklist for VS Code pane UI/CEF behavior. Built-in fallback launches use `~/Library/Application Support/GSDE/Chromium`. The `profile` field is currently accepted for config compatibility but configured browser panes intentionally share one project profile. `GSDE_BROWSER_PANES` accepts 1-4 browser panes for smoke-test overrides. Initial browser URLs can be overridden with comma-separated `GSDE_BROWSER_URLS`:
